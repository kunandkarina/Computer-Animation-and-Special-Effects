    ...
    if (ImGui::Begin("Control")) {
        ...
        if (ImGui::BeginCombo("Animation", current_animation)) {
            for (int n = 0; n < IM_ARRAYSIZE(animations); n++) {
                bool is_selected =
                    (current_animation ==
                     animations[n]);  // You can store your selection however you want, outside or inside your objects
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        for (const auto& ballptr : targets) {
            ...
            if (ballptr->rayIntersectsSphere(currentCamera->getPosition().head<3>(), rayDirection, 0.150)) {
                ...
                current_end_bone_index =
                    std::distance(targets.begin(), std::find(targets.begin(), targets.end(), ballptr));
...
void cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
    for (const auto& ballptr : targets) {
        if (ballptr->dragging) {
            rayDirection = screenToWorldRay(window, xpos, ypos);
            Eigen::Vector4d newPos = calculateIntersectionPoint(currentCamera->getPosition().head<3>(), rayDirection,
